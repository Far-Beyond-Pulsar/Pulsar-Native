...
fn generate_crate_index(crate_dir: &Path, docs: &CrateDocumentation) -> Result<(), Box<dyn Error>> {
    ...
    if !docs.type_aliases.is_empty() {
        writeln!(
            md,
            "- [Type Aliases](type_aliases/) ({} items)",
            docs.type_aliases.len()
        )?;
    }
...
fn generate_type_aliases_pages(
    crate_dir: &Path,
