...
fn main() {
    ...
    tracing::trace!("cargo:warning=[pulsar_docs] Workspace root: {:?}", workspace_root);
    ...
    tracing::trace!("cargo:warning=[pulsar_docs] AUTO_GENERATE_DOCS = {}", auto_generate);
    ...
    if !doc_dir.exists() {
        ...
        if let Err(e) = fs::create_dir_all(&doc_dir) {
            tracing::trace!("cargo:warning=[pulsar_docs] Warning: Could not create doc directory: {}", e);
        }
    }
    ...
    if has_json_files {
        tracing::trace!("cargo:warning=[pulsar_docs] Documentation already exists, skipping generation");
        tracing::trace!("cargo:warning=[pulsar_docs] To regenerate docs, delete target/doc and rebuild");
    ...
    match doc_generator::generate_workspace_docs(workspace_root, &doc_dir) {
        Ok(count) => {
            tracing::trace!("cargo:warning=[pulsar_docs] ✓ Successfully generated docs for {} crates", count);
        }
        Err(e) => {
            tracing::trace!("cargo:warning=[pulsar_docs] ✗ Failed to generate docs: {}", e);
            tracing::trace!("cargo:warning=[pulsar_docs] Documentation will be unavailable in the UI");
        }
    }
