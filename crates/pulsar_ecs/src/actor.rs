...
/// # Lifecycle
///
/// 1. `begin_play` — called once when registered (after the world is ready).
/// 2. `tick` — called every frame with the current `GameTime`.
/// 3. `end_play` — called once on deregistration or shutdown.
///
/// # Example
///
/// ```
/// use pulsar_ecs::{Actor, World, Entity, GameTime};
///
/// struct Player;
///
...
impl ActorRegistry {
    ...
    /// entry from the registry.  Safe to call multiple times — second call
    /// is a no-op.
