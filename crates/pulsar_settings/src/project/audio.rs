...
pub fn register(cfg: &'static ConfigManager) {
    let schema = NamespaceSchema::new("Audio", "Audio engine and mixer settings")
        ...
        .setting(
            "reverb_enabled",
            SchemaEntry::new(
                "Enable global reverb convolution for environmental sound",
                false,
            )
            .label("Reverb")
            .page("Audio")
        ...
        .setting(
            "hrtf",
            SchemaEntry::new(
                "Use Head-Related Transfer Function for headphone 3D audio",
                false,
            )
            .label("HRTF (Headphones)")
            .page("Audio")
            .field_type(FieldType::Checkbox),
        )
        .setting(
            ...
            SchemaEntry::new(
                "Maximum number of sounds playing at once before eviction",
                64_i64,
            )
        ...
        .setting(
            "hrtf_enabled",
            SchemaEntry::new(
                "Use Head-Related Transfer Function (HRTF) for realistic headphone spatialization",
                false,
            )
            .label("HRTF (Headphone Spatialization)")
            .page("Audio")
            .field_type(FieldType::Checkbox),
        )
        .setting(
            "reverb_enabled",
            SchemaEntry::new(
                "Enable environmental reverb and convolution on audio sources",
                true,
            )
            .label("Reverb")
            .page("Audio")
        ...
        .setting(
            "occlusion_enabled",
            SchemaEntry::new(
                "Occlude sounds that pass through geometry using raycasts",
                false,
            )
            .label("Audio Occlusion")
            .page("Audio")
            .field_type(FieldType::Checkbox),
        )
        .setting(
            "voice_channels",
            SchemaEntry::new(
                "Maximum number of simultaneous voice/sound channels",
                64_i64,
            )
        ...
        .setting(
            "doppler_scale",
            SchemaEntry::new(
                "Doppler effect intensity multiplier (0.0 = off, 1.0 = realistic)",
                0.0_f64,
            )
            .label("Doppler Scale")
            .page("Audio")
        ...
        .setting(
            ...
            SchemaEntry::new("Falloff model for 3D audio attenuation", "inverse")
                .label("Distance Attenuation Model")
                .page("Audio")
                .field_type(FieldType::Dropdown {
                    options: vec![
        ...
        .setting(
            "streaming_threshold_kb",
            SchemaEntry::new(
                "Audio files larger than this value (KB) are streamed from disk",
                256_i64,
            )
            .label("Streaming Threshold (KB)")
            .page("Audio")
            .field_type(FieldType::NumberInput {
                min: Some(64.0),
                max: Some(8192.0),
        ...
        .setting(
            "mix_presets_file",
            SchemaEntry::new(
                "Path to the audio mix presets/busses configuration file",
                "config/audio_mix.toml",
            )
            .label("Mix Presets File")
            .page("Audio")
            .field_type(FieldType::TextInput {
                placeholder: Some("config/audio_mix.toml".into()),
                multiline: false,
            }),
        )
        .setting(
            "duck_on_voice",
            SchemaEntry::new(
                "Automatically duck (reduce) background music volume during voice lines",
                true,
            )
            .label("Duck on Voice")
            .page("Audio")
        ...
        .setting(
            "duck_amount_db",
            SchemaEntry::new(
                "Amount to reduce background music volume during ducking in dB",
                -12.0_f64,
            )
            .label("Duck Amount (dB)")
            .page("Audio")
            .field_type(FieldType::Slider {
                min: -40.0,
                max: 0.0,
