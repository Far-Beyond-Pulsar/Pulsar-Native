...
pub fn register(cfg: &'static ConfigManager) {
    let schema = NamespaceSchema::new("Packaging", "Distribution and packaging settings")
        .setting(
            "staging_dir",
            SchemaEntry::new(
                "Directory where the packaged build is assembled before archiving",
                "dist/staged/",
            )
        ...
        .setting(
            ...
            SchemaEntry::new(
                "Remove editor-only assets and metadata from the package",
                true,
            )
            .label("Strip Editor Content")
            .page("Packaging")
            .field_type(FieldType::Checkbox),
        )
        .setting(
        ...
        .setting(
            ...
            SchemaEntry::new("Code signing certificate identity or fingerprint", "")
                ...
                .field_type(FieldType::TextInput {
                    placeholder: Some("Developer ID Application: Your Name".into()),
                    multiline: false,
                }),
        )
        .setting(
            "bundle_id",
            SchemaEntry::new(
                "macOS / iOS bundle identifier in reverse-DNS format",
                "com.example.mygame",
            )
            .label("Bundle ID")
            .page("Packaging")
            .field_type(FieldType::TextInput {
                placeholder: Some("com.example.mygame".into()),
                multiline: false,
            }),
        )
        .setting(
            "version_in_filename",
            SchemaEntry::new(
                "Append the project version to the output package filename",
                true,
            )
            .label("Version in Filename")
            .page("Packaging")
            .field_type(FieldType::Checkbox),
        )
        .setting(
        ...
        .setting(
            "strip_unused_assets",
            SchemaEntry::new(
                "Remove unreferenced assets from the package to save space",
                true,
            )
            .label("Strip Unused Assets")
            .page("Packaging")
            .field_type(FieldType::Checkbox),
        )
        .setting(
            "cook_on_the_fly_server",
            SchemaEntry::new(
                "Start a content cook-on-the-fly server for testing without full packaging",
                false,
            )
            .label("Cook-on-the-Fly Server")
            .page("Packaging")
            .field_type(FieldType::Checkbox),
        )
        .setting(
            "notarize_macos",
            SchemaEntry::new(
                "Submit macOS builds to Apple notarization after code signing",
                false,
            )
        ...
        .setting(
            "installer_type",
            SchemaEntry::new(
                "Type of installer/package to generate for desktop targets",
                "none",
            )
            .label("Installer Type")
            .page("Packaging")
        ...
        .setting(
            "run_after_package",
            SchemaEntry::new(
                "Automatically launch the packaged build for quick smoke testing",
                false,
            )
            .label("Run After Package")
            .page("Packaging")
            .field_type(FieldType::Checkbox),
        )
        .setting(
            "packaging_threads",
            SchemaEntry::new(
                "Number of parallel threads used for packaging and cooking (0 = auto)",
                0_i64,
            )
            .label("Packaging Threads")
            .page("Packaging")
