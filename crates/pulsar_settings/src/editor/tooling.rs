...
pub fn register(cfg: &'static ConfigManager) {
    let schema = NamespaceSchema::new("Tooling", "Editor productivity and workflow settings")
        ...
        .setting(
            "restore_layout_on_open",
            SchemaEntry::new(
                "Restore the previous editor layout when opening a project",
                true,
            )
        ...
        .setting(
            ...
            SchemaEntry::new(
                "Custom build command run when build system is set to 'custom'",
                "",
            )
            .label("Custom Build Command")
            .page("Tooling")
            .field_type(FieldType::TextInput {
                placeholder: Some("./build.sh --release".into()),
                multiline: false,
            }),
        )
        .setting(
            "lsp_enabled",
            SchemaEntry::new(
                "Enable the built-in Language Server Protocol client for code intelligence",
                true,
            )
            .label("LSP Client")
            .page("Tooling")
        ...
        .setting(
            "rust_analyzer_path",
            SchemaEntry::new(
                "Path to a custom rust-analyzer binary (empty = PATH lookup)",
                "",
            )
            .label("rust-analyzer Binary")
            .page("Tooling")
            .field_type(FieldType::TextInput {
                placeholder: Some("/usr/bin/rust-analyzer".into()),
                multiline: false,
            }),
        )
        .setting(
        ...
        .setting(
            "formatter_command",
            SchemaEntry::new(
                "Custom formatter command run on save (empty = rustfmt for Rust files)",
                "",
            )
            .label("Formatter Command")
            .page("Tooling")
            .field_type(FieldType::TextInput {
                placeholder: Some("rustfmt --edition 2021".into()),
                multiline: false,
            }),
        )
        .setting(
            "clippy_on_save",
            SchemaEntry::new(
                "Run Clippy after each save and surface lints as diagnostics",
                false,
            )
            .label("Clippy on Save")
            .page("Tooling")
            .field_type(FieldType::Checkbox),
        )
        .setting(
            "test_runner",
            SchemaEntry::new(
                "Test framework used by the integrated test explorer",
                "cargo-test",
            )
            .label("Test Runner")
            .page("Tooling")
        ...
        .setting(
            ...
            SchemaEntry::new("Custom test command when test runner is 'custom'", "")
                ...
                .page("Tooling")
                .field_type(FieldType::TextInput {
                    placeholder: Some("./run_tests.sh".into()),
                    multiline: false,
                }),
        )
        .setting(
            ...
            SchemaEntry::new(
                "Launch the external profiler alongside game-in-editor runs",
                false,
            )
            .label("Profiler Integration")
            .page("Tooling")
            .field_type(FieldType::Checkbox),
        )
        .setting(
            "task_shortcuts",
            SchemaEntry::new(
                "Path to a JSON file defining custom task shortcuts shown in the toolbar",
                "",
            )
            .label("Task Shortcuts File")
            .page("Tooling")
            .field_type(FieldType::TextInput {
                placeholder: Some("config/tasks.json".into()),
                multiline: false,
            }),
        );
