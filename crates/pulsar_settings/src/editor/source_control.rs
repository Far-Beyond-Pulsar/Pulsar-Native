    ...
    let schema = NamespaceSchema::new("Source Control", "Integrated version control settings")
        ...
        .setting("use_git_lfs",
            SchemaEntry::new("Track large binary files with Git LFS", false)
                ...
                .field_type(FieldType::Checkbox))
        .setting("diff_tool",
            SchemaEntry::new("External diff tool invoked for 3-way merge conflicts", "builtin")
                .label("Diff / Merge Tool").page("Source Control")
                .field_type(FieldType::Dropdown { options: vec![
                    DropdownOption::new("Built-in", "builtin"),
        ...
        .setting("custom_diff_command",
            SchemaEntry::new("Command for the custom diff tool ($BASE $LOCAL $REMOTE $MERGED tokens)", "")
                .label("Custom Diff Command").page("Source Control")
                .field_type(FieldType::TextInput { placeholder: Some("meld $LOCAL $BASE $REMOTE --output $MERGED".into()), multiline: false }))
        .setting("commit_template_file",
            SchemaEntry::new("Path to a commit message template file", "")
                .label("Commit Template File").page("Source Control")
                .field_type(FieldType::TextInput { placeholder: Some(".gitmessage".into()), multiline: false }))
        .setting("sign_commits",
            SchemaEntry::new("GPG/SSH sign commits automatically", false)
                .label("Sign Commits (GPG/SSH)").page("Source Control")
                .field_type(FieldType::Checkbox))
        .setting("default_branch_name",
            SchemaEntry::new("Default branch name for new repositories", "main")
                .label("Default Branch Name").page("Source Control")
                .field_type(FieldType::TextInput { placeholder: Some("main".into()), multiline: false }))
        .setting("rebase_on_pull",
            SchemaEntry::new("Rebase local commits on top of the remote instead of creating a merge commit", false)
                .label("Rebase on Pull").page("Source Control")
                .field_type(FieldType::Checkbox))
        .setting("prune_on_fetch",
            SchemaEntry::new("Automatically prune deleted remote refs during fetch", true)
                .label("Prune on Fetch").page("Source Control")
                .field_type(FieldType::Checkbox))
        .setting("show_blame_inline",
            SchemaEntry::new("Show Git blame information inline in the text editor", false)
                .label("Inline Blame").page("Source Control")
                .field_type(FieldType::Checkbox))
        .setting("blame_date_format",
            SchemaEntry::new("Date format used in inline blame annotations", "relative")
                .label("Blame Date Format").page("Source Control")
                .field_type(FieldType::Dropdown { options: vec![
        ...
        .setting("show_file_status_icons",
            SchemaEntry::new("Show Git status icons on files in the project tree", true)
                .label("File Status Icons").page("Source Control")
                .field_type(FieldType::Checkbox))
        .setting("max_history_entries",
            SchemaEntry::new("Maximum number of commits shown in the history panel", 500_i64)
                .label("Max History Entries").page("Source Control")
                .field_type(FieldType::NumberInput { min: Some(50.0), max: Some(10000.0), step: Some(50.0) })
