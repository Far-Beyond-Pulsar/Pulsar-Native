...
fn expand_primitive_alias(
    ...
    if has_deprecated_primitive.is_some() || has_deprecated_structure.is_some() {
        emit_deprecation_warning(has_deprecated_primitive, has_deprecated_structure);
    }
...
fn emit_deprecation_warning(
    ...
    // Pick the most useful span for the header location line (first deprecated arg found)
