...
pub fn create_shared_textures_startup(
    ...
    tracing::debug!("[BEVY] 🔧 create_shared_textures_startup() CALLED - Creating placeholder Images...");
    tracing::debug!("[BEVY] 📍 This runs in STARTUP schedule (main world) BEFORE rendering begins");
        ...
        if let Some(ref textures) = *lock {
            if let Ok(native_lock) = textures.native_handles.lock() {
                if native_lock.is_some() {
                    tracing::debug!("[BEVY] ⚠️  Textures already created, skipping");
                    return;
                }
            }
        }
    ...
    tracing::debug!("[BEVY] 📦 Creating placeholder Images (actual DXGI textures created in render world)...");
    ...
    tracing::debug!("[BEVY] ✅ Placeholder render target Images created");
}
...
pub fn create_shared_textures_startup(
    ...
    tracing::debug!("[BEVY] 🔧 Creating shared textures (non-Windows)...");

    // Check if already created
    if let Ok(lock) = shared_textures.0.lock() {
        if lock.is_some() {
            tracing::debug!("[BEVY] ⚠️ Textures already created");
            return;
        }
    }
    ...
    tracing::debug!("[BEVY] ✅ Render target Images created");
}
...
pub fn create_shared_textures(
    ...
    tracing::debug!("[BEVY] 🔧 create_shared_textures() CALLED - Replacing render targets with DXGI shared textures...");
    tracing::debug!("[BEVY] 📍 This should only run ONCE on the first render cycle");

    let texture_handles = match shared_textures.0.lock().ok().and_then(|l| l.as_ref().map(|t| t.textures.clone())) {
        Some(handles) => handles,
        None => {
            tracing::debug!("[BEVY] ❌ No texture handles available");
            return;
        }
    };
    ...
    let d3d12_device = unsafe {
        match render_device.wgpu_device().as_hal::<Dx12>() {
            ...
            None => {
                tracing::debug!("[BEVY] ❌ Failed to get D3D12 device");
                return;
            }
        }
    };
    ...
    let tex_0 = unsafe {
        match crate::subsystems::render::DxgiSharedTexture::create(&d3d12_device, RENDER_WIDTH, RENDER_HEIGHT, DXGI_FORMAT_B8G8R8A8_UNORM) {
            Ok(t) => t,
            Err(e) => {
                tracing::debug!("[BEVY] ❌ Failed to create texture 0: {}", e);
                return;
            }
        }
    };
    ...
    let tex_1 = unsafe {
        match crate::subsystems::render::DxgiSharedTexture::create(&d3d12_device, RENDER_WIDTH, RENDER_HEIGHT, DXGI_FORMAT_B8G8R8A8_UNORM) {
            Ok(t) => t,
            Err(e) => {
                tracing::debug!("[BEVY] ❌ Failed to create texture 1: {}", e);
                return;
            }
        }
    };
    ...
    tracing::debug!("[BEVY] ✅ Created DXGI textures: 0x{:X}, 0x{:X}", handle_0, handle_1);
    tracing::debug!("[BEVY] 🎯 Handles will now be stored for GPUI compositor access");
        ...
        if let Some(ref mut textures) = *lock {
            if let Ok(mut handles_lock) = textures.native_handles.lock() {
                *handles_lock = Some([
                ...
                tracing::debug!("[BEVY] 💾 Stored native handles in SharedGpuTextures");
            }
        }
    }
    ...
    unsafe {
        ...
        tracing::debug!("[BEVY] 📍 Injecting DXGI texture into asset ID 0: {:?}", texture_handles[0].id());
        tracing::debug!("[BEVY] 📍 Injecting DXGI texture into asset ID 1: {:?}", texture_handles[1].id());
        gpu_images.insert(&texture_handles[0], gpu_img_0);
        gpu_images.insert(&texture_handles[1], gpu_img_1);

        tracing::debug!("[BEVY] ✅ Injected DXGI textures into Bevy - Rendering DIRECTLY to shared GPU memory!");
