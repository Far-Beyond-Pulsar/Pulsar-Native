...
pub fn ensure_core_bootstrap(project_root: &Path) -> Result<(), String> {
    ...
    let classes_mod = classes_dir.join("mod.rs");
    {
        ...
        if let Ok(entries) = std::fs::read_dir(&classes_dir) {
        ...
        std::fs::write(&classes_mod, mod_content)
...
fn ensure_engine_primitives(project_root: &Path) {
    ...
    let Ok(entries) = std::fs::read_dir(&src_prims) else {
        return;
    };
    for entry in entries.flatten() {
        let src = entry.path();
        if src.extension().and_then(|e| e.to_str()) != Some("fbx") {
            continue;
        }
