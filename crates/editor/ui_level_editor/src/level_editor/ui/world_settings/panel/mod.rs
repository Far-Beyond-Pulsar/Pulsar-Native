//! World Settings Panel implementation.
//!
//! Renders every world/scene setting (environment, GI, fog, physics, audio)
//! through replication-aware inputs that synchronize across multiuser
//! sessions. This is the panel body — it renders *under* the dock wrapper
//! `WorldSettingsPanel` in `workspace::panels`, which owns the entity, the
//! frame pump, and the `ui::dock::Panel` plumbing.
    ...
    fn create_f32_input(
        ...
        let field_sub =
            cx.subscribe_in(&input, window, move |_panel, _state, event, _window, cx| {
                tracing::info!("WorldSettingsPanelImpl: Input event {:?}", event);
                if matches!(
                    event,
