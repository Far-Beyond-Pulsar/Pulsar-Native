...
impl CloudService {
    ...
    pub fn fetch_server_info(
        ...
        let tok: Option<String> = if token.is_empty() {
            None
        } else {
                    ...
                    if let Ok(envs) = r.json::<serde_json::Value>() {
                        if let Some(arr) = envs.as_array() {
                            let chosen = arr
                                .iter()
                                .find(|e| {
                                    e.get("is_root").and_then(|v| v.as_bool()).unwrap_or(false)
                                })
                                .or_else(|| arr.first());
    ...
    pub fn open_workspace(
        ...
        let token_opt: Option<String> = if auth_token.is_empty() {
            None
        } else {
        ...
        let ctx = if !auth_token.is_empty() {
            ctx.with_auth_token(auth_token.to_string())
        } else {
            ctx
        };
        ...
        let ws_eid = if environment_id.is_empty() {
            None
        } else {
