...
use crate::utils::caller_tracking::{refresh_snapshot, CallerRow, CALLER_SNAPSHOT};
use crate::utils::tracking_allocator::{disable_tracking, enable_tracking, is_tracking_active};
use gpui::*;
use std::rc::Rc;
use ui::{
...
impl CallerSitesPanel {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        let filter_input =
            cx.new(|cx| InputState::new(window, cx).placeholder("Filter by symbol…"));
    ...
    fn apply_sort(&mut self) {
        ...
        match self.sort_col {
            ...
            SortCol::Allocs => self.cached_rows.sort_unstable_by(|a, b| {
                let c = a.total_allocs.cmp(&b.total_allocs);
                if asc {
                    c
                } else {
                    c.reverse()
            ...
            SortCol::Deallocs => self.cached_rows.sort_unstable_by(|a, b| {
                let c = a.total_deallocs.cmp(&b.total_deallocs);
                if asc {
                    c
                } else {
                    c.reverse()
            ...
            SortCol::Live => self.cached_rows.sort_unstable_by(|a, b| {
                let c = a.live_bytes.cmp(&b.live_bytes);
                if asc {
                    c
                } else {
                    c.reverse()
            ...
            SortCol::Total => self.cached_rows.sort_unstable_by(|a, b| {
                let c = a.total_bytes.cmp(&b.total_bytes);
                if asc {
                    c
                } else {
                    c.reverse()
            ...
            SortCol::EstLeak => self.cached_rows.sort_unstable_by(|a, b| {
                let c = a.leaked_estimate.cmp(&b.leaked_estimate);
                if asc {
                    c
                } else {
                    c.reverse()
    ...
    fn fmt_live(bytes: i64) -> String {
        if bytes < 0 {
            format!("-{}", Self::fmt_bytes((-bytes) as u64))
        } else {
            Self::fmt_bytes(bytes as u64)
        }
    }
}
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Refresh rows from snapshot and re-apply sort.
        {
            ...
            self.cached_rows = if filter_text.is_empty() {
                ...
                snap.iter()
                    .filter(|r| r.symbol.to_lowercase().contains(&f))
                    .cloned()
                    .collect()
            };
        }
        ...
        let (tot_allocs, tot_deallocs, tot_live, tot_bytes, tot_leak) = self
            .cached_rows
            .iter()
            .fold((0u64, 0u64, 0i64, 0u64, 0u64), |(a, d, l, b, k), r| {
                (
                    a + r.total_allocs,
                    d + r.total_deallocs,
                    l + r.live_bytes,
        ...
        let live_total_color = if tot_live < 0 {
            theme.danger
        } else {
            theme.success
        };
        let leak_total_color = if tot_leak > 0 {
            theme.danger
        } else {
            theme.muted_foreground
        };
        ...
        v_flex()
                ...
                h_flex()
                    ...
                    .child(
                        div()
                            ...
                            .child(format!(
                                "process live: {}",
                                CallerSitesPanel::fmt_live(global_live)
                            )),
                    )
                    .child(
                        // Toggle button
            ...
            .child(
                h_flex()
                    ...
                    .child(num_cell(
                        CallerSitesPanel::fmt_live(tot_live),
                        live_total_color,
                    ))
                    .child(num_cell(
                        CallerSitesPanel::fmt_bytes(tot_bytes),
                        theme.warning,
                    ))
                    .child(num_cell(
                        CallerSitesPanel::fmt_bytes(tot_leak),
                        leak_total_color,
                    )),
            )
            // ── Virtual list ──────────────────────────────────────────────────
                    ...
                    move |_this, range, _window, cx| {
                        ...
                        range
                            .map(|ix| {
                                ...
                                let live_color = if row.live_bytes < 0 {
                                    theme.danger
                                } else {
                                    theme.success
                                };
                                let leak_color = if row.leaked_estimate > 0 {
                                    theme.danger
                                } else {
                                    theme.muted_foreground
                                };
                                ...
                                h_flex()
                                    ...
                                    .child(num_cell(
                                        format!("{}", row.total_allocs),
                                        theme.muted_foreground,
                                    ))
                                    .child(num_cell(
                                        format!("{}", row.total_deallocs),
                                        theme.muted_foreground,
                                    ))
                                    .child(num_cell(
                                        CallerSitesPanel::fmt_live(row.live_bytes),
                                        live_color,
                                    ))
                                    .child(num_cell(
                                        CallerSitesPanel::fmt_bytes(row.total_bytes),
                                        theme.warning,
                                    ))
                                    .child(num_cell(
                                        CallerSitesPanel::fmt_bytes(row.leaked_estimate),
                                        leak_color,
                                    ))
                                    .into_any_element()
                            })
