...
use crate::utils::system_info::SharedSystemInfo;
...
impl Render for GpuMetricsPanel {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let color_vram = if vram_pct > 85.0 {
            theme.danger
        } else if vram_pct > 65.0 {
            theme.warning
        } else {
            theme.success
        };
        ...
        v_flex()
                                ...
                                h_flex()
                                    ...
                                    .child(
                                        div()
                                            .text_size(px(10.0))
                                            .text_color(theme.muted_foreground)
                                            .child("Dedicated Used"),
                                    )
                                    .child(
                                        div()
                                            .text_size(px(11.0))
                                            .font_weight(gpui::FontWeight::BOLD)
                                            .text_color(color_vram)
                                            .child(format!(
                                                "{:.0} MiB ({:.0}%)",
                                                vram_used_mb, vram_pct
                                            )),
                                    ),
                            )
                            .child(
            ...
            .when(!engines.is_empty(), |this: Div| {
                this.child(div().w_full().grid().grid_cols(4).gap_2().children(
                    engines.into_iter().map(|(name, hist)| {
