...
pub struct TypeTracker {
    ...
    pub(crate) db_writer: Mutex<Option<Arc<crate::utils::memory_database::MemoryDatabaseWriter>>>,
}
...
impl TypeTracker {
    ...
    pub fn init_db_writer(&self) {
        ...
        match crate::utils::memory_database::get_memory_db_path() {
            Ok(db_path) => {
                match crate::utils::memory_database::MemoryDatabaseWriter::new(db_path.clone()) {
                    Ok(writer) => {
...
fn background_processor(receiver: Receiver<AllocRecord>) {
    ...
    loop {
        ...
        if !pending_records.is_empty() && last_write.elapsed().as_secs() >= 5 {
            ...
            let sites = TYPE_TRACKER.get_sites();

            if let Some(writer) = TYPE_TRACKER.db_writer.lock().as_ref() {
