...
fn main() {{
    ...
    blueprints::register_compiled_classes(&mut game.world, &mut game.actors);
...
//! [`pulsar_game::Actor`], and calling `register_compiled_classes` at startup.

{mod_decls}
{use_decls}
use pulsar_game::{{ActorRegistry, Entity, World}};

/// Register every compiled blueprint class with the game world.
///
/// Called once at startup before the tick loop begins.
pub fn register_compiled_classes(world: &mut World, actors: &mut ActorRegistry) {{
{register_body}}}
