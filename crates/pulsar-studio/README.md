...
`pulsar-studio` is a self-hosted collaboration server that stores Pulsar Engine project files and coordinates real-time editing sessions between multiple team members. It exposes a REST API consumed by the Pulsar launcher and an editor WebSocket channel for live collaboration.

## Quick Start

```bash
# Build
cargo build -p pulsar-studio --release

# Run with defaults (port 7700, data stored in ./pulsar-studio-data/)
./target/release/pulsar-studio
