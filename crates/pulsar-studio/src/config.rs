...
/// Pulsar Studio — dedicated multi-user project server for Pulsar Engine studios.
#[derive(Parser, Debug, Clone)]
#[command(name = "pulsar-studio", version, about)]
pub struct Cli {
    /// TCP port to listen on.
    #[arg(long, env = "PULSAR_STUDIO_PORT", default_value = "7700")]
    pub port: u16,

    /// Bind address.
    #[arg(long, env = "PULSAR_STUDIO_BIND", default_value = "0.0.0.0")]
    pub bind: String,

    /// Directory used to store project files and metadata.
    #[arg(
        long,
        env = "PULSAR_STUDIO_DATA_DIR",
        default_value = "./pulsar-studio-data"
    )]
    pub data_dir: PathBuf,

    /// Human-readable name shown to connected Pulsar clients.
    #[arg(long, env = "PULSAR_STUDIO_NAME", default_value = "Pulsar Studio Server")]
    pub server_name: String,

    /// Bearer token required for write operations (and reads if set).
    /// Leave empty for an open / development server.
    #[arg(long, env = "PULSAR_STUDIO_AUTH_TOKEN", default_value = "")]
    pub auth_token: String,

    /// Maximum number of projects this server will host.
    #[arg(long, env = "PULSAR_STUDIO_MAX_PROJECTS", default_value = "100")]
    pub max_projects: usize,

    /// Log filter directive, e.g. "info", "debug", "pulsar_studio=trace".
