...
impl ChatProvider for OpenAiChatProvider {
    ...
    fn chat_streaming(
        ...
        let response = req
            .send()
            .with_context(|| format!("chat {}", self.chat_url))?;
...
impl OpenAiChatProvider {
    ...
    fn ollama_models(&self) -> anyhow::Result<Vec<ModelDescriptor>> {
        let resp = self
            ...
            .get(&self.models_url)
            .send()
            .with_context(|| format!("ollama tags from {}", self.models_url))?;
        if !resp.status().is_success() {
            return Err(anyhow!(
                "ollama tags API {}: {}",
                resp.status(),
...
fn parse_ollama_response(raw: Value) -> anyhow::Result<ChatResponse> {
    ...
    let fr = raw
        ...
        .map(|s| s.to_string())
        .or_else(|| {
            raw.get("done").and_then(|v| v.as_bool()).and_then(|d| {
                if d {
                    Some("stop".into())
                } else {
                    None
...
fn read_ollama_stream(
    ...
    for line in BufReader::new(response).lines() {
        ...
        if let Some(e) = ev.get("error").and_then(|v| v.as_str()) {
        ...
        if fr.is_none() && ev.get("done").and_then(|v| v.as_bool()).unwrap_or(false) {
            fr = Some("stop".into());
        }
