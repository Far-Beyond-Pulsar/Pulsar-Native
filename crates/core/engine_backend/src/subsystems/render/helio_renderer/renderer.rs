...
impl HelioRenderer {
    ...
    /// Pushes `entity`'s `#[gpu]`-mirrored `LightComponentGpuMirror`
    /// (auto-generated by `#[engine_class]` from `LightComponent`'s own
    /// `#[gpu]`-marked sub-props fields, Pulsar-Native#561 -- no longer a
    /// hand-rolled companion type) into Helio's own light storage, or
    /// removes it if the entity doesn't currently have one. Called once per
    /// full component re-dispatch -- the same granularity `LightComponent::
    ...
    /// and an object with no light at all does one cheap
    /// `get::<LightComponentGpuMirror>` miss here and nothing more.
    ///
    /// `position_range`'s xyz is overwritten from `snap.transform.position`
    /// here -- the mirrored row's own copy is a zeroed placeholder by design
    /// (see `LightComponentGpuMirror::to_helio_gpu_light`'s doc); this is
    /// the one place that combines it with the real, current transform
    /// before use.
    fn sync_light_gpu_data(
        ...
        match store.world().get::<LightComponentGpuMirror>(entity) {
            Some(mirror) => {
                let mut gpu = mirror.to_helio_gpu_light();
            ...
            None => {
                ...
                // its light is disabled (hydrate already removed the
                // mirror in that case). Drop any stale Helio actor left
                // over from a previous enabled state.
