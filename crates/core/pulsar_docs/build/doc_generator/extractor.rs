...
fn extract_source_from_span<T: ToTokens>(node: &T, _source_code: &str) -> String {
    ...
    if let Ok(mut file) = syn::parse_file(&tokens.to_string()) {
        ...
        file.items
            .retain(|item| !matches!(item, syn::Item::Verbatim(_)));
...
) -> Result<CrateDocumentation, Box<dyn Error>> {
    ...
    for item in items {
        match item {
            Item::Struct(item_struct) => {
                docs.structs.push(extract_struct(
                    item_struct,
                    file_path,
                    &current_path,
                    source_code,
            ...
            Item::Trait(item_trait) => {
                docs.traits.push(extract_trait(
                    item_trait,
                    file_path,
                    &current_path,
                    source_code,
            ...
            Item::Fn(item_fn) => {
                docs.functions.push(extract_function(
                    item_fn,
                    file_path,
                    &current_path,
                    source_code,
            ...
            Item::Macro(item_macro) => {
                docs.macros.push(extract_macro(
                    item_macro,
                    file_path,
                    &current_path,
                    source_code,
            ...
            Item::Const(item_const) => {
                docs.constants.push(extract_constant(
                    item_const,
                    file_path,
                    &current_path,
                    source_code,
            ...
            Item::Type(item_type) => {
                docs.type_aliases.push(extract_type_alias(
                    item_type,
                    file_path,
                    &current_path,
                    source_code,
    ...
    let variants = item
        ...
        .map(|v| {
            let fields = match &v.fields {
                syn::Fields::Named(fields) => VariantFields::Struct(
                    fields
                        .named
                        .iter()
                        .map(|f| FieldDoc {
                ...
                syn::Fields::Unnamed(fields) => VariantFields::Tuple(
                    fields
                        .unnamed
                        .iter()
...
fn extract_generics(generics: &syn::Generics) -> Vec<Generic> {
    generics
        ...
        .filter_map(|param| match param {
            syn::GenericParam::Type(type_param) => Some(Generic {
