...
pub fn blueprint(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    // For generic functions the types contain unbound type params (e.g. Vec<T>) which are not
    // in scope in a `static` item, so fall back to 0/8 for those.
    ...
    // For generic functions: size=0, align=8 (types not in scope in a static item).
    let (return_type, return_size, return_align) = match &input.sig.output {
        ...
        ReturnType::Type(_, ty) => {
            ...
            if ty_trimmed == "()" || ty_trimmed == "!" {
                ...
                (quote! { Some(#ty_str) }, quote! { 0usize }, quote! { 8usize })
            } else {
                (
                    quote! { Some(#ty_str) },
                    quote! { ::std::mem::size_of::<#ty>() },
...
fn generate_dispatch_shim(
    ...
    // Skip generic functions — no concrete type to dispatch
    ...
    quote! {
        #[cfg(not(target_arch = "wasm32"))]
        #[no_mangle]
        pub unsafe extern "C" fn #shim_ident(args: *const *const u8, ret: *mut u8) {
            #body
        }
    }
}
