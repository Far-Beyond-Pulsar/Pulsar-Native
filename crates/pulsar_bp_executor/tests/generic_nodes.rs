...
fn add_node(id: &str, a: i64, b: i64) -> NodeInstance {
    ...
    n.properties.insert(format!("{id}_a"), PropertyValue::Number(a as f64));
    n.properties.insert(format!("{id}_b"), PropertyValue::Number(b as f64));
    n
}

fn array_new_node(id: &str) -> NodeInstance {
    let mut n = NodeInstance::new(id, "array_new", Position::default());
    n.outputs.push(PinInstance::new(&format!("{id}_r"), Pin::new(&format!("{id}_r"), "result", DataType::Typed(graphy::TypeInfo::new("Vec")), PinType::Output)));
    n
}
...
fn array_push_node(id: &str) -> NodeInstance {
    ...
    n.inputs.push(PinInstance::new(&format!("{id}_arr"), Pin::new(&format!("{id}_arr"), "array", DataType::Typed(graphy::TypeInfo::new("Vec")), PinType::Input)));
    n.inputs.push(PinInstance::new(&format!("{id}_itm"), Pin::new(&format!("{id}_itm"), "item",  DataType::Typed(graphy::TypeInfo::new("i64")), PinType::Input)));
    n.outputs.push(PinInstance::new(&format!("{id}_eo"), Pin::new(&format!("{id}_eo"), "exec",   DataType::Execution,                                   PinType::Output)));
    n.outputs.push(PinInstance::new(&format!("{id}_r"),  Pin::new(&format!("{id}_r"),  "result", DataType::Typed(graphy::TypeInfo::new("Vec")), PinType::Output)));
    n
}
...
fn array_push_vec_item_node(id: &str) -> NodeInstance {
    ...
    n.inputs.push(PinInstance::new(&format!("{id}_e"),   Pin::new(&format!("{id}_e"),   "exec",  DataType::Execution,                                   PinType::Input)));
    n.inputs.push(PinInstance::new(&format!("{id}_arr"), Pin::new(&format!("{id}_arr"), "array", DataType::Typed(graphy::TypeInfo::new("Vec")), PinType::Input)));
    n.inputs.push(PinInstance::new(&format!("{id}_itm"), Pin::new(&format!("{id}_itm"), "item",  DataType::Typed(graphy::TypeInfo::new("Vec")), PinType::Input)));
    n.outputs.push(PinInstance::new(&format!("{id}_eo"), Pin::new(&format!("{id}_eo"), "exec",   DataType::Execution,                                   PinType::Output)));
    n.outputs.push(PinInstance::new(&format!("{id}_r"),  Pin::new(&format!("{id}_r"),  "result", DataType::Typed(graphy::TypeInfo::new("Vec")), PinType::Output)));
    n
}
...
fn array_set_node(id: &str) -> NodeInstance {
    ...
    n.inputs.push(PinInstance::new(&format!("{id}_e"),   Pin::new(&format!("{id}_e"),   "exec",  DataType::Execution,                                   PinType::Input)));
    n.inputs.push(PinInstance::new(&format!("{id}_arr"), Pin::new(&format!("{id}_arr"), "array", DataType::Typed(graphy::TypeInfo::new("Vec")), PinType::Input)));
    n.inputs.push(PinInstance::new(&format!("{id}_idx"), Pin::new(&format!("{id}_idx"), "index", DataType::Typed(graphy::TypeInfo::new("i64")), PinType::Input)));
    n.inputs.push(PinInstance::new(&format!("{id}_val"), Pin::new(&format!("{id}_val"), "value", DataType::Typed(graphy::TypeInfo::new("i64")), PinType::Input)));
    n.outputs.push(PinInstance::new(&format!("{id}_eo"), Pin::new(&format!("{id}_eo"), "exec",   DataType::Execution,                                   PinType::Output)));
    n.outputs.push(PinInstance::new(&format!("{id}_r"),  Pin::new(&format!("{id}_r"),  "result", DataType::Typed(graphy::TypeInfo::new("Vec")), PinType::Output)));
    n
}
