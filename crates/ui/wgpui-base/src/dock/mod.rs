//! Dockable layout: splits and tab groups that a host can rearrange,
//! persist, and restore — with no appearance of its own.
//!
...
//! [`NodeId`], panels by [`PanelId`], and no gpui_ce entity handle is stored
...
//! variant, so a panel can only ever live inside a `Tabs`.
//!
...
//! [`DockContext`] and [`TabGroupContext`] — each of which
...
//!   be installed after construction, which in gpui_ce means a deferred pass.
...
//! use wgpui_base::dock::{DockArea, DockLayout, DockPlacement};
//!
//! let area = cx.new(|cx| {
...
//! renderers included — run it with `cargo run -p gpui_ce-base dock`.
