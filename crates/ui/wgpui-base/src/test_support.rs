//! Opt-in headless observation built exclusively on public gpui_ce APIs.
//!
//! Native accessibility properties are read automatically. Applications register
//! identified gpui_ce elements with `.test_support()`, without supplying test-only state.
use gpui::{
...
impl ElementSnapshot {
    ...
    pub fn focused(&self) -> Option<bool> {
        assert!(
            ...
            "focus binding was not observed for {:?}; use .test_support().track_focus(&handle). gpui_ce does not expose pre-existing or implicit focus handles",
            self.path
        );
        self.focused
    }
...
/// Internal lookup used by Kit's testing API. Scope follows gpui_ce's element path.
#[doc(hidden)]
...
/// Transparent forwarding element. Its registration is owned by gpui_ce element
...
impl<E: Element> Observed<E> {
    ...
    /// Changes the native identity while keeping observation outside gpui_ce's
