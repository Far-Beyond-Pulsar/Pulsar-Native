...
//! [`App::reduce_motion`], but gpui_ce never reads the platform's setting into
...
//! application inherits the user's choice by calling `wgpui_base::init` or
//! `gpui_component::init`.
//!
...
//! flag only while it still holds what Base last wrote (or gpui_ce's initial
...
//!
//! Under gpui_ce's test scheduler the platform is never consulted, whichever
...
/// Applies the system's reduced-motion preference during `wgpui_base::init`.
...
///
/// `wgpui_base::init` calls this once. Call it again to re-read the preference
...
/// session without the portal) the flag is left as it is, and under gpui_ce's
...
/// Whether the application runs on gpui_ce's deterministic test scheduler,
