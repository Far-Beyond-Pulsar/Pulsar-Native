...
fn time<T>(label: &str, f: impl FnOnce() -> T) -> T {
    let t = Instant::now();
    let r = f();
    println!("[timing] {:45} {:>10.3} ms", label, t.elapsed().as_secs_f64() * 1000.0);
    r
}
...
fn make_deep_graph(n: usize) -> GraphDescription {
    ...
    for i in 0..n {
        ...
        if i > 0 {
            ...
            g.add_connection(Connection::new(&prev, &format!("{prev}_r"), &id, &format!("{id}_a"), ConnectionType::Data));
        }
    }
    let last = format!("a{}", n - 1);
    g.add_node(gt_node("gt", None, Some(0.0)));
    g.add_node(branch_node("br"));
    g.add_connection(Connection::new(&last, &format!("{last}_r"), "gt", "gt_a", ConnectionType::Data));
    g.add_connection(Connection::new("begin", "be", "br", "br_e", ConnectionType::Execution));
    g.add_connection(Connection::new("gt", "gt_r", "br", "br_c", ConnectionType::Data));
    g
}
...
fn test_embedded_lib_is_non_empty() {
    assert!(!PULSAR_STD_LIB_BYTES.is_empty(), "embedded lib must not be empty");
    println!("[info] PULSAR_STD_LIB embedded: {} bytes ({:.1} KiB), ext={}",
...
fn test_end_to_end_add_3_plus_4_is_7() {
    ...
    g.add_connection(Connection::new("begin", "be", "chk", "chk_e", ConnectionType::Execution));
...
fn test_end_to_end_branch_graph() {
    ...
    g.add_node(gt_node("gt", None, Some(0.0)));
    g.add_node(branch_node("br"));
    g.add_connection(Connection::new("a", "a_r", "gt", "gt_a", ConnectionType::Data));
    g.add_connection(Connection::new("begin", "be", "br", "br_e", ConnectionType::Execution));
    g.add_connection(Connection::new("gt", "gt_r", "br", "br_c", ConnectionType::Data));
    compile_and_run(&exec, &g);
}
...
fn test_multiple_event_nodes_produce_separate_programs() {
    ...
    bp.outputs.push(PinInstance::new("bp_e", Pin::new("bp_e", "Body", DataType::Execution, PinType::Output)));
    let mut main_ev = NodeInstance::new("main_ev", "main", Position { x: 200.0, y: 0.0 });
    main_ev.outputs.push(PinInstance::new("main_e", Pin::new("main_e", "Body", DataType::Execution, PinType::Output)));
    g.add_node(bp);
    g.add_node(main_ev);
...
fn test_timing_bytecode_vs_rust_codegen() {
    ...
    println!("[timing] {:45} {:>10.3} ms  ({} chars)", "50-node → Rust codegen",
        rust_t.as_secs_f64() * 1000.0, code.len());

    let t1 = Instant::now();
    let progs = compile_graph_to_bytecode(&g).unwrap();
    let bc_t = t1.elapsed();
    println!("[timing] {:45} {:>10.3} ms  ({} instrs)", "50-node → bytecode compile",
        bc_t.as_secs_f64() * 1000.0, progs[0].instructions.len());

    println!("[timing] speedup: {:.1}×", rust_t.as_secs_f64() / bc_t.as_secs_f64().max(1e-9));
}
