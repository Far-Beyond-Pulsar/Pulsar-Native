...
impl ChatProvider for MistralProvider {
    ...
    fn availability(&self, env: &dyn ProviderEnvironment) -> ProviderAvailability {
        if Self::auth_token_from_env(env).is_some() {
            ...
            ProviderAvailability::requires_auth("Authentication required. Set MISTRAL_API_KEY.")
        }
    }
        ...
        match method {
            AuthMethod::PromptToken => {
                let token = host.prompt_for_token(PromptTokenRequest {
                    title: "Mistral Authentication".to_string(),
                    prompt: "Paste your Mistral API key.".to_string(),
                    placeholder: Some("...".to_string()),
                    env_var_hint: Some("MISTRAL_API_KEY".to_string()),
                })?;
    ...
    fn chat_completion(&self, token: &str, request: &ChatRequest) -> anyhow::Result<ChatResponse> {
        ...
        if !response.status().is_success() {
            ...
            return Err(anyhow!("Mistral API returned {}: {}", status, body));
        }

        let raw_response: Value = response.json().context("invalid JSON from Mistral API")?;
    ...
    fn chat_completion_streaming(
        ...
        let response = self
            ...
            .json(&payload)
            .send()
            .context("failed to call Mistral streaming chat API")?;

        if !response.status().is_success() {
            let status = response.status();
            let body = response.text().unwrap_or_default();
            return Err(anyhow!(
                "Mistral streaming API returned {}: {}",
                status,
                body
            ));
        }
        ...
        for line in reader.lines() {
            let line = line.context("failed reading Mistral streaming response line")?;
            let line = line.trim();
            if line.is_empty() {
                continue;
            }
            ...
            let event: Value =
                serde_json::from_str(data).context("invalid JSON event in Mistral stream")?;
