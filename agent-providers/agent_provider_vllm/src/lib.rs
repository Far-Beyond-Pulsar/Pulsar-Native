...
const CHAT_URL: &str = "http://localhost:8000/v1/chat/completions";
...
impl ChatProvider for VllmProvider {
    ...
    fn chat_completion(&self, _token: &str, request: &ChatRequest) -> anyhow::Result<ChatResponse> {
        ...
        let response = self
            ...
            .json(&payload)
            .send()
            .context("failed to call vLLM chat API")?;

        if !response.status().is_success() {
            let status = response.status();
            let body = response.text().unwrap_or_default();
            return Err(anyhow!("vLLM API returned {}: {}", status, body));
        }

        let raw_response: Value = response.json().context("invalid JSON from vLLM API")?;
    ...
    fn chat_completion_streaming(
        ...
        let response = self
            ...
            .json(&payload)
            .send()
            .context("failed to call vLLM streaming chat API")?;
        ...
        for line in reader.lines() {
            let line = line.context("failed reading vLLM streaming response line")?;
            let line = line.trim();
            if line.is_empty() {
                continue;
            }
            ...
            let event: Value =
                serde_json::from_str(data).context("invalid JSON event in vLLM stream")?;
