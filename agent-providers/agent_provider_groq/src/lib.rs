...
impl ChatProvider for GroqProvider {
    ...
    fn availability(&self, env: &dyn ProviderEnvironment) -> ProviderAvailability {
        if Self::auth_token_from_env(env).is_some() {
            ...
            ProviderAvailability::requires_auth("Authentication required. Set GROQ_API_KEY.")
        }
    }
        ...
        match method {
            AuthMethod::PromptToken => {
                let token = host.prompt_for_token(PromptTokenRequest {
                    title: "Groq Authentication".to_string(),
                    prompt: "Paste your Groq API key.".to_string(),
                    placeholder: Some("gsk_...".to_string()),
                    env_var_hint: Some("GROQ_API_KEY".to_string()),
                })?;
    ...
    fn chat_completion(&self, token: &str, request: &ChatRequest) -> anyhow::Result<ChatResponse> {
        ...
        let response = self
            ...
            .json(&payload)
            .send()
            .context("failed to call Groq chat API")?;
        ...
        let raw_response: Value = response.json().context("invalid JSON from Groq API")?;
    ...
    fn chat_completion_streaming(
        ...
        let response = self
            ...
            .json(&payload)
            .send()
            .context("failed to call Groq streaming chat API")?;
        ...
        for line in reader.lines() {
            let line = line.context("failed reading Groq streaming response line")?;
            let line = line.trim();
            if line.is_empty() {
                continue;
            }
