...
impl ChatProvider for TogetherProvider {
    ...
    fn availability(&self, env: &dyn ProviderEnvironment) -> ProviderAvailability {
        if Self::auth_token_from_env(env).is_some() {
            ...
            ProviderAvailability::requires_auth("Authentication required. Set TOGETHER_API_KEY.")
        }
    }
        ...
        match method {
            AuthMethod::PromptToken => {
                let token = host.prompt_for_token(PromptTokenRequest {
                    title: "Together AI Authentication".to_string(),
                    prompt: "Paste your Together API key.".to_string(),
                    placeholder: Some("...".to_string()),
                    env_var_hint: Some("TOGETHER_API_KEY".to_string()),
                })?;
    ...
    fn chat_completion(&self, token: &str, request: &ChatRequest) -> anyhow::Result<ChatResponse> {
        ...
        let response = self
            .client
            .post(TOGETHER_CHAT_URL)
            .header("Authorization", format!("Bearer {token}"))
            .header("Content-Type", "application/json")
            .json(&payload)
            .send()
            .context("failed to call Together chat API")?;

        if !response.status().is_success() {
            let status = response.status();
            let body = response.text().unwrap_or_default();
            return Err(anyhow!("Together API returned {}: {}", status, body));
        }

        let raw_response: Value = response.json().context("invalid JSON from Together API")?;
    ...
    fn chat_completion_streaming(
        ...
        let response = self
            ...
            .json(&payload)
            .send()
            .context("failed to call Together streaming chat API")?;

        if !response.status().is_success() {
            let status = response.status();
            let body = response.text().unwrap_or_default();
            return Err(anyhow!(
                "Together streaming API returned {}: {}",
                status,
                body
            ));
        }
        ...
        for line in reader.lines() {
            let line = line.context("failed reading Together streaming response line")?;
            let line = line.trim();
            if line.is_empty() {
                continue;
            }
            ...
            let event: Value =
                serde_json::from_str(data).context("invalid JSON event in Together stream")?;
