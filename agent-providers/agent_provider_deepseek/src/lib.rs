...
const CHAT_URL: &str = "https://api.deepseek.com/v1/chat/completions";
...
impl ChatProvider for DeepSeekProvider {
    ...
    fn availability(&self, env: &dyn ProviderEnvironment) -> ProviderAvailability {
        if Self::auth_token_from_env(env).is_some() {
            ...
            ProviderAvailability::requires_auth("Authentication required. Set DEEPSEEK_API_KEY.")
        }
    }
        ...
        match method {
            AuthMethod::PromptToken => {
                let token = host.prompt_for_token(PromptTokenRequest {
                    title: "DeepSeek Authentication".to_string(),
                    prompt: "Paste your DeepSeek API key.".to_string(),
                    placeholder: Some("sk-...".to_string()),
                    env_var_hint: Some("DEEPSEEK_API_KEY".to_string()),
                })?;
    ...
    fn chat_completion(&self, token: &str, request: &ChatRequest) -> anyhow::Result<ChatResponse> {
        ...
        let response = self
            ...
            .json(&payload)
            .send()
            .context("failed to call DeepSeek chat API")?;

        if !response.status().is_success() {
            let status = response.status();
            let body = response.text().unwrap_or_default();
            return Err(anyhow!("DeepSeek API returned {}: {}", status, body));
        }

        let raw_response: Value = response.json().context("invalid JSON from DeepSeek API")?;
    ...
    fn chat_completion_streaming(
        ...
        let response = self
            ...
            .json(&payload)
            .send()
            .context("failed to call DeepSeek streaming chat API")?;

        if !response.status().is_success() {
            let status = response.status();
            let body = response.text().unwrap_or_default();
            return Err(anyhow!(
                "DeepSeek streaming API returned {}: {}",
                status,
                body
            ));
        }
        ...
        for line in reader.lines() {
            let line = line.context("failed reading DeepSeek streaming response line")?;
            let line = line.trim();
            if line.is_empty() {
                continue;
            }
            ...
            let event: Value =
                serde_json::from_str(data).context("invalid JSON event in DeepSeek stream")?;
