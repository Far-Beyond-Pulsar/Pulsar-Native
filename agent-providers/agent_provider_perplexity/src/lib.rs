...
const CHAT_URL: &str = "https://api.perplexity.ai/chat/completions";
...
impl PerplexityProvider {
    ...
    fn auth_token_from_env(env: &dyn ProviderEnvironment) -> Option<String> {
        env.get_env("PERPLEXITY_API_KEY")
    }
...
impl ChatProvider for PerplexityProvider {
    ...
    fn availability(&self, env: &dyn ProviderEnvironment) -> ProviderAvailability {
        if Self::auth_token_from_env(env).is_some() {
            ...
            ProviderAvailability::requires_auth("Authentication required. Set PERPLEXITY_API_KEY.")
        }
    }
        ...
        match method {
            AuthMethod::PromptToken => {
                let token = host.prompt_for_token(PromptTokenRequest {
                    title: "Perplexity Authentication".to_string(),
                    prompt: "Paste your Perplexity API key.".to_string(),
                    placeholder: Some("pplx-...".to_string()),
                    env_var_hint: Some("PERPLEXITY_API_KEY".to_string()),
                })?;
    ...
    fn chat_completion(&self, token: &str, request: &ChatRequest) -> anyhow::Result<ChatResponse> {
        ...
        let response = self
            ...
            .json(&payload)
            .send()
            .context("failed to call Perplexity chat API")?;

        if !response.status().is_success() {
            let status = response.status();
            let body = response.text().unwrap_or_default();
            return Err(anyhow!("Perplexity API returned {}: {}", status, body));
        }

        let raw_response: Value = response
            .json()
            .context("invalid JSON from Perplexity API")?;
    ...
    fn chat_completion_streaming(
        ...
        let response = self
            ...
            .json(&payload)
            .send()
            .context("failed to call Perplexity streaming chat API")?;

        if !response.status().is_success() {
            let status = response.status();
            let body = response.text().unwrap_or_default();
            return Err(anyhow!(
                "Perplexity streaming API returned {}: {}",
                status,
                body
            ));
        }
        ...
        for line in reader.lines() {
            let line = line.context("failed reading Perplexity streaming response line")?;
            let line = line.trim();
            if line.is_empty() {
                continue;
            }
            ...
            let event: Value =
                serde_json::from_str(data).context("invalid JSON event in Perplexity stream")?;
