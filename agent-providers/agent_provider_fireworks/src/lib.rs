...
impl ChatProvider for FireworksProvider {
    ...
    fn availability(&self, env: &dyn ProviderEnvironment) -> ProviderAvailability {
        if Self::auth_token_from_env(env).is_some() {
            ...
            ProviderAvailability::requires_auth("Authentication required. Set FIREWORKS_API_KEY.")
        }
    }
        ...
        match method {
            AuthMethod::PromptToken => {
                let token = host.prompt_for_token(PromptTokenRequest {
                    title: "Fireworks AI Authentication".to_string(),
                    prompt: "Paste your Fireworks AI API key.".to_string(),
                    placeholder: Some("fw_...".to_string()),
                    env_var_hint: Some("FIREWORKS_API_KEY".to_string()),
                })?;
    ...
    fn chat_completion(&self, token: &str, request: &ChatRequest) -> anyhow::Result<ChatResponse> {
        ...
        let response = self
            ...
            .json(&payload)
            .send()
            .context("failed to call Fireworks AI chat API")?;

        if !response.status().is_success() {
            let status = response.status();
            let body = response.text().unwrap_or_default();
            return Err(anyhow!("Fireworks AI API returned {}: {}", status, body));
        }

        let raw_response: Value = response
            .json()
            .context("invalid JSON from Fireworks AI API")?;
    ...
    fn chat_completion_streaming(
        ...
        let response = self
            ...
            .json(&payload)
            .send()
            .context("failed to call Fireworks AI streaming chat API")?;

        if !response.status().is_success() {
            let status = response.status();
            let body = response.text().unwrap_or_default();
            return Err(anyhow!(
                "Fireworks AI streaming API returned {}: {}",
                status,
                body
            ));
        }
        ...
        for line in reader.lines() {
            let line = line.context("failed reading Fireworks AI streaming response line")?;
            let line = line.trim();
            if line.is_empty() {
                continue;
            }
            ...
            let event: Value =
                serde_json::from_str(data).context("invalid JSON event in Fireworks AI stream")?;
