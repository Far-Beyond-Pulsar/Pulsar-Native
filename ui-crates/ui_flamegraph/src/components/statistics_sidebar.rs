...
pub fn render_statistics_sidebar(
    ...
    let min_frame_time = frame
        .frame_times_ms
        .iter()
        .copied()
        .min_by(|a, b| a.partial_cmp(b).unwrap())
        .unwrap_or(0.0);
    let max_frame_time = frame
        .frame_times_ms
        .iter()
        .copied()
    ...
    let result = div()
        ...
        .child(
            ...
            div()
                ...
                .py_3()
                .gap_2p5()
                .child(stat_row_improved(
                    t!("Flamegraph.TotalSpans").to_string(),
                ...
                .child(stat_row_improved(
                    t!("Flamegraph.MaxDepth").to_string(),
                    format!("{}", frame.max_depth),
                    theme,
                )),
        )
        .child(
            // Divider
            div().h(px(1.0)).w_full().bg(theme.sidebar_border),
        )
        .child(
            // Frame stats section
            div()
                ...
                .child(stat_row_improved(
                    t!("Flamegraph.Frames").to_string(),
                    format!("{}", num_frames),
                    theme,
                ))
                .child(stat_row_improved(
                    t!("Flamegraph.AvgFrame").to_string(),
                    format!("{:.2} ms", avg_frame_time),
                    theme,
                ))
                .child(stat_row_improved(
                    t!("Flamegraph.MinFrame").to_string(),
                    format!("{:.2} ms", min_frame_time),
                    theme,
                ))
                .child(stat_row_improved(
                    t!("Flamegraph.MaxFrame").to_string(),
                    format!("{:.2} ms", max_frame_time),
                    theme,
                )),
        )
        .child(
        ...
        .child(
            // Threads list
            div().flex().flex_col().px_4().pb_4().gap_2().children(
...
fn stat_row_improved(label: String, value: String, theme: &ui::theme::Theme) -> impl IntoElement {
    div()
        .flex()
        .items_center()
        .justify_between()
