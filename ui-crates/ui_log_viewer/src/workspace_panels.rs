...
impl Render for MemoryBreakdownPanel {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let cached_alloc   = total_current;
        ...
        let opt_mb = |v: Option<u64>| v.map(mb_str).unwrap_or_else(|| "N/A".to_string());

        #[derive(Clone)] struct Pt { i: usize, v: f64 }

        let committed_pts: Vec<Pt> = committed_hist.iter().copied().enumerate().map(|(i,v)| Pt{i,v}).collect();
        let cached_pts: Vec<Pt>    = cached_hist.iter().copied().enumerate().map(|(i,v)| Pt{i,v}).collect();
                    ...
                    move |_this, range, _window, cx| {
                        ...
                        range.map(|ix| {
                            if let Some(entry) = entries.get(ix) {
                                let percentage = if total > 0 { (entry.size as f64 / total as f64) * 100.0 } else { 0.0 };
                                    ...
                                    .child(
                                        h_flex().w_full().justify_between().items_center()
                                            ...
                                            .child(h_flex().gap_2().items_center()
                                                ...
                                                .child(div().text_size(px(11.0)).font_weight(gpui::FontWeight::SEMIBOLD)
                                                    .text_color(color).child(format!("{:.1}%", percentage)))
                                            )
                                    )
                                    .child(
                                        div().w_full().h(px(6.0)).bg(theme.border).rounded(px(3.0))
                                            .child(div().h_full().w(relative(percentage as f32 / 100.0))
...
// ═══════════════════════════════════════════════════════════════════════════
// GPU Metrics Panel — per-engine utilization, VRAM, static info
// ═══════════════════════════════════════════════════════════════════════════

/// Advanced GPU metrics: per-engine utilization charts, VRAM breakdown,
/// and static hardware info.
pub struct GpuMetricsPanel {
    focus_handle: FocusHandle,
    metrics: SharedPerformanceMetrics,
    system_info: crate::system_info::SharedSystemInfo,
}
...
impl Render for GpuMetricsPanel {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        engines.sort_by_key(|(k, _)| {
            crate::gpu_engines::KNOWN_ENGINES
                .iter()
                .position(|e| *e == k)
                .unwrap_or(999)
        });
        ...
        let vram_pct = if vram_total_mb > 0.0 {
            (vram_used_mb / vram_total_mb * 100.0).min(100.0)
        } else {
            0.0
        };
        ...
        v_flex()
            ...
            .child(
                div()
                    ...
                    .child(
                        v_flex().gap_1()
                            .child(gpu_info_row("Name", gpu_name, &theme))
                            .child(gpu_info_row("Vendor", gpu_vendor, &theme))
                            .child(gpu_info_row("Driver", gpu_driver, &theme))
                            .child(gpu_info_row("VRAM Total", vram_total, &theme))
                    )
            )
            // ── VRAM ─────────────────────────────────────────────────────────
            ...
            .child(Self::section_header(
                &format!("GPU Engines ({} active)", engines.len()),
                cx,
            ))
                ...
                this.child(
                    div()
                        ...
                        .children(engines.into_iter().map(|(name, hist)| {
                            let current = hist.last().copied().unwrap_or(0.0);
                            let color = if current > 80.0 { theme.danger } else if current > 50.0 { theme.warning } else if current > 5.0 { theme.info } else { theme.muted_foreground };
...
fn gpu_info_row(label: &str, value: String, theme: &ui::ThemeColor) -> impl IntoElement {
    use ui::h_flex;
    h_flex()
        .gap_2()
