...
pub fn render_commit_detail(git_manager: &GitManager, cx: &mut Context<GitManager>) -> impl IntoElement {
    ...
    for file in &git_manager.selected_commit_files {
        ...
        let is_selected = git_manager.selected_commit_file.as_deref()
            .map(|s| s == file.path.as_str()).unwrap_or(false);
        ...
        file_list = file_list.child(
            h_flex()
                ...
                .child(div().text_xs().font_weight(FontWeight::BOLD).text_color(status_color).child(file_status))
                .child(div().flex_1().text_xs().text_color(foreground).overflow_hidden().child(display_name)),
        );
    }
    ...
    // ── File diff (right sub-panel) ───────────────────────────────────────────
    let content_header_label = git_manager.selected_commit_file.clone()
    ...
    let content_body: AnyElement = match &git_manager.selected_commit_file {
        None => v_flex()
            ...
            .child(div().text_xs().text_color(muted_fg).child("Select a file to view its diff"))
            .into_any_element(),

        Some(_) => match (&git_manager.commit_file_diff, &git_manager.commit_file_diff_error) {
            (Some(diff), _) => {
                render_diff_segments(diff, &git_manager.commit_file_expanded, "commit-diff", true, cx)
                    .into_any_element()
            }
