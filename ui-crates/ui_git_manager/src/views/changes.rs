...
use std::rc::Rc;
use ui::{
    ActiveTheme as _, Icon, IconName, StyledExt, VirtualListScrollHandle, v_virtual_list,
...
pub fn render_changes_view(
    ...
    let has_staged = git_manager
        .changes_rows
        .iter()
        .any(|r| matches!(r, ChangesRow::File { is_staged: true, .. }));
        ...
        move |git_manager: &mut GitManager, range: std::ops::Range<usize>, _window: &mut Window, cx: &mut Context<GitManager>| {
            ...
            range.map(|i| -> AnyElement {
                match &git_manager.changes_rows[i].clone() {
                    ChangesRow::Header { title, count, is_staged } => {
        ...
        .when(has_no_changes, |this| {
            this.child(
                v_flex()
                    ...
                    .justify_center()
                    .gap_2()
                    .child(Icon::new(IconName::Check).size(px(24.)).text_color(muted_fg))
                    .child(
                        div()
                            .text_xs()
                            .text_color(muted_fg)
