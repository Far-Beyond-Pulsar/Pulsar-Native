...
use crate::filter::{
    Diagnostic, DiagnosticSeverity, Hint, DiffLineType, compute_aligned_diff,
};
...
impl ProblemsDrawer {
    ...
    pub(crate) fn render_grouped_view(
        ...
        for file_path in files {
            ...
            let file_error_count = diagnostics.iter()
                .filter(|d| matches!(d.severity, DiagnosticSeverity::Error)).count();
            let file_warning_count = diagnostics.iter()
                .filter(|d| matches!(d.severity, DiagnosticSeverity::Warning)).count();
            ...
            let file_group = v_flex()
                ...
                .child(
                    div()
                        .w_full().px_3().py_2().mb_2().rounded_md()
                        .bg(cx.theme().secondary.opacity(0.3))
                        .border_1().border_color(cx.theme().border.opacity(0.3))
                        .child(
                            h_flex().w_full().gap_3().items_center()
                                .child(ui::Icon::new(IconName::Folder)
                                    .size_4().text_color(cx.theme().accent))
                                .child(div().flex_1().text_sm()
                                    .font_weight(gpui::FontWeight::SEMIBOLD)
                                    .text_color(cx.theme().foreground)
                                    .child(display_path.clone()))
                                .child(
                                    h_flex().gap_2()
                                        .when(file_error_count > 0, |this| this.child(
                                            h_flex().gap_1().items_center().px_2().py_0p5()
                                                .rounded_sm()
                                                .bg(DiagnosticSeverity::Error.color(cx).opacity(0.15))
                                                .child(ui::Icon::new(IconName::Close).size_3()
                                        ...
                                        .when(file_warning_count > 0, |this| this.child(
                                            h_flex().gap_1().items_center().px_2().py_0p5()
                                                .rounded_sm()
                                                .bg(DiagnosticSeverity::Warning.color(cx).opacity(0.15))
                                                .child(ui::Icon::new(IconName::TriangleAlert).size_3()
        ...
        F: Fn(&mut Window, &mut App) + 'static,
    {
        ...
        let mut main = v_flex()
            .gap_2().w_full()
            .child(
                h_flex().gap_3().items_center().w_full()
                    .child(
                        h_flex().gap_1p5().items_center()
                            .child(ui::Icon::new(diagnostic.severity.icon())
                                .size_4().text_color(diagnostic.severity.color(cx)))
                            .child(div().text_xs().font_weight(gpui::FontWeight::SEMIBOLD)
                                .text_color(diagnostic.severity.color(cx))
                                .child(diagnostic.severity.label()))
                    )
                    .child(div().flex_1().text_xs().font_family("monospace")
                        .text_color(cx.theme().muted_foreground)
                        .child(format!("{}:{}", diagnostic.line, diagnostic.column)))
                    .when_some(diagnostic.source.as_ref(), |this, source| {
                        this.child(div().px_1p5().py_0p5().rounded_sm().bg(cx.theme().border)
                            .text_xs().font_family("monospace")
                            .text_color(cx.theme().muted_foreground)
                            .child(source.clone()))
                    })
            )
            .child(div().w_full().text_sm().text_color(cx.theme().foreground)
                .line_height(rems(1.4)).child(diagnostic.message.clone()));

        if diagnostic.loading_actions {
            main = main.child(
                h_flex().gap_2().items_center().mt_2().p_2().rounded_md().bg(cx.theme().secondary)
                    .child(Indicator::new().with_size(ui::Size::Small)
        ...
        if !diagnostic.hints.is_empty() && !diagnostic.loading_actions {
            let mut hints_container = v_flex().gap_2().w_full().mt_2()
                .child(div().text_xs().font_weight(gpui::FontWeight::BOLD)
                    .text_color(cx.theme().muted_foreground)
        ...
        if !diagnostic.subitems.is_empty() {
            let mut subitems_container = v_flex().gap_1().w_full()
                .child(div().text_xs().font_weight(gpui::FontWeight::BOLD)
                    .text_color(cx.theme().muted_foreground)
                    ...
                    div().pl_4().py_1().child(
                        v_flex().gap_1()
                            .child(h_flex().gap_2().items_center()
                                .child(ui::Icon::new(sub.severity.icon()).size_3()
                                    .text_color(sub.severity.color(cx)))
                                .child(div().text_xs().font_family("monospace")
                                    .text_color(cx.theme().muted_foreground)
        ...
        div().w_full().px_3().py_2().child(
            div().w_full().px_4().py_3().rounded_lg().border_1()
                .border_color(if is_selected { cx.theme().accent } else { cx.theme().border.opacity(0.5) })
                .bg(if is_selected { cx.theme().accent.opacity(0.08) } else { cx.theme().sidebar.opacity(0.5) })
                .shadow_sm()
                .when(is_selected, |this| this.border_l_3().border_color(cx.theme().accent))
                .hover(|this| this.bg(cx.theme().secondary.opacity(0.7))
                    .border_color(cx.theme().accent.opacity(0.5)))
                .cursor_pointer()
                .on_mouse_down(gpui::MouseButton::Left, move |_, _window, cx| { on_click(_window, cx); })
                .child(main)
        )
    }
    ...
    pub(crate) fn render_hint_diff(
        ...
        if hint.before_content.is_none() && hint.after_content.is_none() {
            return v_flex().gap_1().w_full().px_3().py_2().rounded_md()
                .border_1().border_color(cx.theme().border).bg(cx.theme().sidebar)
                .child(div().text_xs().text_color(cx.theme().foreground)
        ...
        fn build_side(
            ...
            deleted_bg: Hsla, deleted_text: Hsla,
            added_bg: Hsla, added_text: Hsla,
            spacer_bg: Hsla,
            unchanged_bg: Hsla, unchanged_text: Hsla,
            line_num_color: Hsla,
            is_left: bool,
        ) -> Div {
            ...
            for (line_num, line_type, content) in lines {
                ...
                container = container.child(
                    h_flex().w_full().h(px(20.0)).bg(bg)
                        .child(div().w(px(40.0)).h_full().flex().items_center().justify_end()
                            .pr_2().text_xs().font_family("JetBrains Mono")
                            .text_color(line_num_color)
                            .child(if *line_type == DiffLineType::Spacer { String::new() } else {
                                line_num.map(|n| n.to_string()).unwrap_or_default()
                            }))
                        .child(div().flex_1().h_full().flex().items_center().pl_2()
                            .text_xs().font_family("JetBrains Mono")
                            .text_color(text_color).overflow_x_hidden()
                            .child(content.clone()))
                );
            }
            container
        ...
        let left_container = build_side(&left_lines,
            deleted_bg, deleted_text, added_bg, added_text,
            spacer_bg, unchanged_bg, unchanged_text, line_num_color, true);
        let right_container = build_side(&right_lines,
            deleted_bg, deleted_text, added_bg, added_text,
            spacer_bg, unchanged_bg, unchanged_text, line_num_color, false);

        let total_lines = left_lines.len().max(right_lines.len());
        let content_height = (total_lines as f32 * 20.0).max(40.0);

        v_flex().gap_0().w_full().rounded_lg().border_1()
            .border_color(cx.theme().accent.opacity(0.3))
            .bg(cx.theme().background.opacity(0.5))
            .overflow_hidden().shadow_md()
            .child(div().px_4().py_2p5().border_b_1().border_color(cx.theme().border)
                .bg(cx.theme().accent.opacity(0.05))
                .child(h_flex().gap_2().items_center()
                    .child(ui::Icon::new(IconName::Info).size_4().text_color(cx.theme().accent))
                    .child(div().flex_1().text_sm().font_weight(gpui::FontWeight::MEDIUM)
                        .text_color(cx.theme().foreground).child(hint.message.clone()))
                ))
            .child(h_flex().w_full()
                .child(div().w_1_2().min_w_0().px_3().py_1p5()
                    .border_r_1().border_b_1().border_color(cx.theme().border.opacity(0.6))
                ...
                .child(div().w_1_2().min_w_0().px_3().py_1p5()
                    .border_b_1().border_color(cx.theme().border.opacity(0.6))
                    .bg(Hsla { h: 120.0, s: 0.4, l: 0.12, a: 1.0 })
                    .child(h_flex().gap_1p5().items_center()
                        .child(ui::Icon::new(IconName::Check).size_3()
            ...
            .child(div().id("diff-scroll-container").w_full().h(px(content_height))
                .overflow_y_scroll().overflow_x_hidden()
                .child(h_flex().w_full()
                    .child(div().w_1_2().min_w_0().overflow_hidden()
                        .border_r_1().border_color(cx.theme().border.opacity(0.4))
                        .child(left_container))
                    .child(div().w_1_2().min_w_0().overflow_hidden().child(right_container))
                ))
    }
    ...
    pub(crate) fn render_file_preview(
        ...
        if let Ok(content) = fs::read_to_string(&diagnostic.file_path) {
            ...
            if error_line < lines.len() {
                ...
                let input_state = if let Some(existing) = self.preview_inputs.get(&key) {
                    ...
                    let new_state = cx.new(|cx| {
                        let mut state = InputState::new(window, cx)
                            .code_editor(language).soft_wrap(false).rows(num_lines);
                ...
                return div().w_full().mt_2().rounded_md().border_1()
                    .border_color(cx.theme().border).bg(cx.theme().sidebar).overflow_hidden()
                    .child(TextInput::new(&input_state).w_full().h(px(calculated_height))
                        .font_family("JetBrains Mono").text_size(px(12.0)).border_0());
            }
        }
        div()
    }
