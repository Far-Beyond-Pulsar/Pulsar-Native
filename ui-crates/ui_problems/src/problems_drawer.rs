...
pub use crate::filter::{
    Diagnostic, Hint, DiagnosticSeverity, NavigateToDiagnostic,
};
...
impl ProblemsDrawer {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let search_input = cx.new(|cx| {
            InputState::new(window, cx).placeholder("Search problems...")
        });
        Self {
            focus_handle,
            diagnostics,
    ...
    pub fn update_diagnostic_hints(&mut self, diagnostic_index: usize, new_hints: Vec<Hint>, cx: &mut Context<Self>) {
        {
            ...
            if let Some(diag) = diagnostics.get_mut(diagnostic_index) {
                if !new_hints.is_empty() {
                    for hint in new_hints { diag.hints.push(hint); }
                }
    ...
    pub fn get_diagnostic_info(&self, index: usize) -> Option<(String, usize, usize, usize, usize)> {
        self.diagnostics.lock().unwrap().get(index).map(|d| (
            d.file_path.clone(), d.line, d.column,
            d.end_line.unwrap_or(d.line), d.end_column.unwrap_or(d.column),
        ))
    }
    ...
    pub(crate) fn navigate_to_diagnostic(&mut self, diagnostic: &Diagnostic, cx: &mut Context<Self>) {
...
impl ProblemsDrawer {
    fn render_header(
        ...
        v_flex().w_full().gap_3().px_4().py_3().border_b_1()
            .border_color(cx.theme().border).bg(cx.theme().sidebar)
            .child(
                h_flex().w_full().justify_between().items_center()
                    .child(
                        h_flex().gap_3().items_center()
                            .child(div().text_base().font_weight(gpui::FontWeight::BOLD)
                                .text_color(cx.theme().foreground)
                                .child(t!("Problems.Title").to_string()))
                            .child(h_flex().gap_2().items_center()
                                .when(error_count > 0, |this| this.child(
                                    self.render_severity_badge(DiagnosticSeverity::Error, error_count, cx)))
                                .when(warning_count > 0, |this| this.child(
                                    self.render_severity_badge(DiagnosticSeverity::Warning, warning_count, cx)))
                                .when(info_count > 0, |this| this.child(
                                    self.render_severity_badge(DiagnosticSeverity::Information, info_count, cx)))
                            )
                    )
                    .child(h_flex().gap_2()
                        .child(Button::new("toggle-grouping").ghost().small()
                            .icon(if self.group_by_file { IconName::List } else { IconName::Folder })
                        ...
                        .child(Button::new("clear-all").ghost().small()
                            .icon(IconName::Close)
            ...
            .child(
                h_flex().w_full().gap_2().items_center()
                    ...
                    .child(
                        Button::new("filter-dropdown").ghost().small()
    ...
    fn render_severity_badge(&self, severity: DiagnosticSeverity, count: usize, cx: &App) -> impl IntoElement {
        h_flex().gap_1().items_center().px_2().py_0p5().rounded_md()
            .bg(severity.color(cx).opacity(0.15))
            .child(ui::Icon::new(severity.icon()).size_3().text_color(severity.color(cx)))
            .child(div().text_xs().font_weight(gpui::FontWeight::SEMIBOLD)
                .text_color(severity.color(cx)).child(count.to_string()))
    }

    fn render_empty_state(&self, cx: &App) -> Div {
        div().size_full().child(
            div().size_full().flex().items_center().justify_center().p_8().child(
                v_flex().gap_4().items_center().max_w(px(400.0)).px_6().py_8().rounded_xl()
                    .bg(cx.theme().secondary.opacity(0.2))
                    .border_1().border_color(cx.theme().border.opacity(0.3))
                    .child(div().w(px(64.0)).h(px(64.0)).rounded_full()
                        .bg(cx.theme().success.opacity(0.15))
                        .flex().items_center().justify_center()
                        .child(ui::Icon::new(IconName::Check).size(px(32.0))
                            .text_color(cx.theme().success)))
                    .child(div().text_lg().font_weight(gpui::FontWeight::BOLD)
                        .text_color(cx.theme().foreground).child("No problems detected"))
                    .child(div().text_sm().text_center().text_color(cx.theme().muted_foreground)
