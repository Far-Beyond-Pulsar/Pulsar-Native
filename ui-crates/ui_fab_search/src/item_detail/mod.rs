...
use ui::{
    ...
    reveal_in_file_manager, fmt_speed,
};

use crate::parser::{fmt_count, strip_html, SketchfabModelDetail};
...
impl ItemDetailView {
    pub fn new(
        ...
        Self {
            detail, images,
            scroll_handle, scroll_state,
            gallery_scroll_handle, gallery_scroll_state,
...
impl RenderOnce for ItemDetailView {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let seller_avatar: Option<Arc<gpui::RenderImage>> = d.user.as_ref()
            ...
            .cloned();
        let category: Option<SharedString> = d.categories.first()
        ...
        let stats = ModelStatsSection::new(
            ...
            d.face_count, d.vertex_count,
            d.material_count, d.texture_count,
            d.animation_count, d.sound_count,
            d.pbr_type.clone(),
        );

        // ── sidebar data ─────────────────────────────────────────────────────
        let sidebar_formats: Vec<String> = d.archives.as_ref()
            .map(|a| a.available().into_iter()
                .map(|(lbl, arc)| {
                    let size = arc.size_label().unwrap_or_default();
                    if size.is_empty() { lbl.to_string() }
                    else { format!("{} ({})", lbl, size) }
                }).collect())
            .unwrap_or_default();
        ...
        let left_col = v_flex()
            ...
            .child(
                h_flex()
                    .w_full().px_5().py_4().gap_3().items_center()
                    ...
                    .child(
                        v_flex().flex_1().min_w_0()
                            .child(div().text_sm().font_bold().text_color(fg).truncate().child(seller_name.clone()))
                    ...
                    .child(
                        h_flex().gap_4().items_center()
                            .child(div().text_xs().text_color(muted).child(format!("👁 {}", fmt_count(view_count))))
                            .child(div().text_xs().text_color(muted).child(format!("♥ {}", fmt_count(like_count))))
        ...
        let sidebar = v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        v_flex().flex_1().min_w_0()
                            .child(div().text_sm().font_bold().text_color(fg).truncate().child(seller_name))
            ...
            .child(
                h_flex()
                    ...
                    .when_some(published_at.as_deref(), |el, date| {
                        el.child(mini_stat("Cal", date[..date.len().min(10)].to_string(), fg, muted))
                    })
            )
            // ── license ───────────────────────────────────────────────
            .child(
                v_flex()
                    ...
                    .gap_2()
                    .border_b_1().border_color(border)
                    .child(div().text_xs().font_bold().text_color(muted).child("LICENSE"))
                    .child(
                        h_flex().gap_2().items_center().flex_wrap()
                            .when_some(license_label, |el, label| {
                                el.child(div().text_sm().font_semibold().text_color(fg).child(label))
                            })
                            .when(is_downloadable, |el| {
                                el.child(
                                    div().px_2().py(px(2.0)).rounded_full()
                                        .bg(gpui::rgb(0x14532D))
                                        .text_xs().font_bold().text_color(success)
                                        .child("\u{2713} Free")
                                )
                            })
                            .when(!is_downloadable, |el| {
                                el.child(div().text_xs().text_color(muted).child("Purchase required"))
                            })
                    )
            )
            // ── download CTA ──────────────────────────────────────────
                    ...
                    .map(|section| {
                        if let Some(dl_fn) = on_download {
                            match download_status.as_ref() {
                                Some(DownloadState::InProgress { bytes_received, total_bytes, speed_bps, .. }) => {
                                    let pct = total_bytes
                                        .filter(|&t| t > 0)
                                    ...
                                    section
                                        .child(div().text_xs().font_bold().text_color(muted).child("DOWNLOADING"))
                                        .child(div().text_sm().font_medium().text_color(fg).child(format!("{}{}", pct, speed_str)))
                                }
                                Some(DownloadState::Done { path, .. }) => {
                                    let path = path.clone();
                                    section
                                        .child(div().text_sm().font_bold().text_color(success).child("Downloaded \u{2713}"))
                                        .child(
                                            Button::new("sidebar-open-folder")
                                                .ghost()
                                                .small()
                                                .label("\u{1F4C2} Open Folder")
                                                .on_click(move |_, _, _| { reveal_in_file_manager(&path); })
                                        )
                                }
                                Some(DownloadState::Error { message, .. }) => {
                                    let msg = message.clone();
                                    section
                                        .child(div().text_xs().text_color(gpui::red()).child(format!("Error: {}", msg)))
                                        .child(
                                            Button::new("sidebar-retry")
                                                .ghost().small()
                                                .label("Retry")
                                                .on_click(move |_, w, cx| { (dl_fn)(w, cx); })
                                        )
                                }
                                None => {
                                    section.child(
                            ...
                            section.child(div().text_xs().text_color(muted).child("Sign in to download"))
                        }
                    })
            )
            // ── available formats ─────────────────────────────────────
            .when(!sidebar_formats.is_empty(), |card| {
                card.child(
                    v_flex()
                        ...
                        .gap_2()
                        .border_t_1().border_color(border)
                        .child(div().text_xs().font_bold().text_color(muted).child("AVAILABLE FORMATS"))
                        .child(
                            div().flex().flex_row().flex_wrap().gap_2()
                                .children(sidebar_formats.into_iter().map(|fmt| {
                                    Tag::primary().small().rounded_full().child(fmt)
                                }))
                        )
                )
            })
                ...
                div()
                    ...
                    .child(
                        Button::new("sidebar-view-sketchfab")
                            ...
                            .on_click(move |_, _, cx| { cx.open_url(viewer_url2.as_ref()); })
                    )
            );
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .child(
                                div().w_full().px_6().pt_6().pb_16()
                                    .child(
                                        h_flex()
                                            .w_full().gap_6().items_start()
                                            .child(left_col)
                                            .child(sidebar)
                                    )
                            )
