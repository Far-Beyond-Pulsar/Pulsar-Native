    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let body: AnyElement = if self.is_loading {
            ...
            if self.detail_loading {
                ...
                // ── extract data before listeners ─────────────────────────
                let d_uid        = detail.uid.clone();
                let d_title      = detail.title.clone();
                let d_seller     = detail.user.seller_name.clone();
                let d_category   = detail.category.as_ref().map(|c| c.name.clone()).unwrap_or_default();
                let d_rating     = detail.ratings.as_ref().map(|r|
                    format!("★ {:.1}  ({} ratings)", r.average_rating, r.total)
                ).unwrap_or_default();
                let d_reviews    = detail.review_count.map(|n| format!("{} reviews", n)).unwrap_or_default();
                let d_is_free    = detail.is_free;
                let d_desc_raw   = detail.description.clone().unwrap_or_default();
                let d_desc       = parser::strip_html(&d_desc_raw);
                let d_formats: Vec<String> = detail.asset_formats.iter()
                    .map(|f| f.asset_format_type.name.clone()).collect();
                let d_tags: Vec<String> = detail.tags.iter().map(|t| t.name.clone()).collect();
                let d_licenses: Vec<(String, String)> = detail.licenses.iter().map(|l| {
                    let price_str = if d_is_free {
                        "Free".to_string()
                    } else if let Some(ref pt) = l.price_tier {
                        if let Some(disc) = pt.discounted_price {
                            format!("{}{:.2} (was {}{:.2})", pt.currency_code, disc, pt.currency_code, pt.price)
                        } else {
                            format!("{}{:.2}", pt.currency_code, pt.price)
                        }
                    } else {
                        "—".to_string()
                    };
                    (l.name.clone(), price_str)
                }).collect();
                let d_changelogs: Vec<(String, String)> = detail.changelogs.iter().rev().map(|c| {
                    let date = c.published_at.get(..10).unwrap_or("").to_string();
                    (date, parser::strip_html(&c.content))
                }).collect();
                let d_media_count = detail.medias.len();
                let open_url = format!("https://www.fab.com/i/listings/{}", d_uid);
                ...
                div()
                        ...
                        div()
                            ...
                            .child(
                                div().text_sm().text_color(muted_fg).child(
                                    if d_category.is_empty() { d_seller.clone() }
                                    else { format!("{} · {}", d_seller, d_category) }
                                )
                            )
                            .when(!d_rating.is_empty(), |el| {
                                el.child(
                                    h_flex().gap_3().child(
                                        div().text_sm().text_color(fg).child(d_rating)
                                    ).when(!d_reviews.is_empty(), |e| {
                                        e.child(div().text_sm().text_color(muted_fg).child(d_reviews))
                                    })
                                )
                            })
                    )
                    // ── gallery thumbnails ─────────────────────────────────
                    .when(d_media_count > 0, |el| {
                        el.child(
                            div()
                                .px_4()
                                .py_3()
                                ...
                                .gap_2()
                                .child(
                                    div().text_xs().font_bold().text_color(muted_fg)
                                        .child(format!("Gallery ({} images)", d_media_count))
                                )
                                .child(
                                    div()
                                        .flex()
                                        ...
                                        .gap_2()
                                        .flex_wrap()
                                        .children((0..d_media_count.min(8)).map(|i| {
                                            div()
                                                .w(px(120.0))
                                                .h(px(68.0))
                                                .flex_shrink_0()
                    ...
                    .when(!d_licenses.is_empty(), |el| {
                        el.child(
                            div()
                                ...
                                .gap_2()
                                .child(
                                    div().text_xs().font_bold().text_color(muted_fg).child("Licenses")
                                )
                                .children(d_licenses.iter().map(|(name, price)| {
                                    h_flex()
                                        .gap_3()
                                        .child(div().text_sm().font_bold().text_color(fg).child(name.clone()))
                                        .child(div().text_sm().text_color(muted_fg).child(price.clone()))
                                }))
                        )
                    })
                    // ── formats + tags ─────────────────────────────────────
                    .child(
                        div()
                            ...
                            .flex_col()
                            .gap_2()
                            .when(!d_formats.is_empty(), |el| {
                                el.child(
                                    div()
                                        .flex().flex_row().flex_wrap().gap_2()
                                        .children(d_formats.iter().map(|f| {
                                            div()
                                                .text_xs().px_2().py(px(2.0))
                                                .rounded_sm().bg(accent_bg).text_color(accent_fg)
                                                .child(f.clone())
                                        }))
                                )
                            })
                            .when(!d_tags.is_empty(), |el| {
                                el.child(
                                    div()
                                        .flex().flex_row().flex_wrap().gap_2()
                                        .children(d_tags.iter().map(|t| {
                                            div()
                                                .text_xs().px_2().py(px(2.0))
                                                .rounded_sm().bg(card_bg).border_1()
                                                .border_color(border_col).text_color(muted_fg)
                                                .child(t.clone())
                                        }))
                                )
                            })
                    ...
                    .when(!d_desc.is_empty(), |el| {
                        el.child(
                            div()
                                ...
                                .gap_2()
                                .child(
                                    div().text_xs().font_bold().text_color(muted_fg).child("Description")
                                )
                                .child(
                                    div().text_sm().text_color(fg).child(d_desc)
                                )
                        )
                    })
                    // ── changelogs ─────────────────────────────────────────
                    .when(!d_changelogs.is_empty(), |el| {
                        el.child(
                            div()
                                .px_4()
                                .py_3()
                                ...
                                .gap_2()
                                .child(
                                    div().text_xs().font_bold().text_color(muted_fg).child("Changelog")
                                )
                                .children(d_changelogs.iter().map(|(date, content)| {
                                    v_flex()
                                        .gap_1()
                                        .mb_2()
                                        .child(div().text_xs().text_color(muted_fg).child(date.clone()))
                                        .child(div().text_sm().text_color(fg).child(content.clone()))
                                }))
                        )
                    })
                    .into_any_element()
