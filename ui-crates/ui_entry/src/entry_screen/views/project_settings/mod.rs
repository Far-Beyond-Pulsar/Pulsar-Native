...
fn render_settings_sidebar(
    ...
    v_flex()
        ...
        .child(
            v_flex().flex_1().justify_end().child(
...
fn render_sidebar_item(
    ...
    div()
        ...
        .child(Icon::new(icon).size(px(16.)).text_color(if is_active {
            theme.primary
        } else {
...
fn render_settings_content(
    ...
    v_flex()
        ...
        .p_8()
        .child(match settings.active_tab {
            ProjectSettingsTab::General => {
                general::render_general_tab(settings, cx).into_any_element()
            }
            ProjectSettingsTab::GitInfo => {
                git_info::render_git_info_tab(settings, cx).into_any_element()
            }
            ProjectSettingsTab::GitCI => git_ci::render_git_ci_tab(settings, cx).into_any_element(),
            ProjectSettingsTab::Metadata => {
                metadata::render_metadata_tab(settings, cx).into_any_element()
            }
            ProjectSettingsTab::DiskInfo => {
                disk_info::render_disk_info_tab(settings, cx).into_any_element()
            }
            ProjectSettingsTab::Performance => {
                performance::render_performance_tab(settings, cx).into_any_element()
            }
            ProjectSettingsTab::Integrations => {
                integrations::render_integrations_tab(settings, cx).into_any_element()
            }
        })
}
