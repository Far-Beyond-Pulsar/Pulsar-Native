...
impl ProjectConfig {
    fn from_toml(content: &str) -> Result<Self, String> {
        ...
        let project_table = parsed
            .get("project")
            .and_then(|v| v.as_table())
            .ok_or("Missing [project] section")?;

        let settings_table = parsed.get("settings").and_then(|v| v.as_table());
        ...
        Ok(Self {
            name: project_table
                ...
                .unwrap_or("Unknown")
                .to_string(),
            version: project_table
                .get("version")
                .and_then(|v| v.as_str())
                .unwrap_or("0.1.0")
                .to_string(),
            engine_version: project_table
                .get("engine_version")
                .and_then(|v| v.as_str())
                .unwrap_or("Unknown")
                .to_string(),
...
"#,
            self.name, self.version, self.engine_version, self.default_scene
        )
    }
}
    ...
    v_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .child(div().text_sm().text_color(theme.foreground).child(
                                "Failed to parse Pulsar.toml. Please check the file format.",
                            )),
                    ),
            )
        })
        .child(render_info_section(
            ...
            vec![
                (
                    "Project Root",
                    settings.project_path.to_string_lossy().to_string(),
                ),
                (
                    "Assets Folder",
                    settings
