...
|---|---|---|---|
| 1 | Delta-sync replaces per-frame copy-and-reconcile | **MET** | T4 centerpiece: **5.2–5.9× CPU** speedup at realistic partial-mutation rates (0.1–1%), **100–1000× fewer bytes** moved, at all three scales tested (1k/10k/100k rows). Conservative (legacy modeled as a lower bound — see §1.2). Loses only at the degenerate M=100% (whole-scene-every-frame) case, by 25–30%, and that loss is real per-row bookkeeping economics (~13–15 ns/row scenedb vs ~10–11 ns/row legacy), not an artifact — see §3.1. |
...
| 21 | Multi-view queries scale by threading | **DEFERRED** | Test 1 covers thread-safety; no scaling bench across concurrent views was built this campaign — not one of T1–T7's task list. |
