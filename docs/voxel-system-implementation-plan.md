...
1. **SceneDB owns all state that must survive a frame, graph rebuild, cache eviction, save/load, undo, or replication.** This includes component configuration, generator/source descriptions, material-ID lists, canonical edits, and any generated voxel payload that is itself the authoritative result. If it must be recoverable after renderer cache loss, it cannot exist only in the renderer.
2. **Renderer/pass state is transient or deterministically rebuildable only.** GPU bricks, extracted surfaces, acceleration structures, residency/page tables, staging allocations, in-flight tasks, visibility, and camera-relative coordinates are cache/work state. Never make them the unique owner of authored state, edits, chunk identity, or persistence journals.
...
9. **Keep APIs generic and composable.** External scripts/tools must be able to create/configure components and publish large voxel/chunk batches through a stable SceneDB-facing service/API, without reaching into pass internals or requiring a renderer lock.
...
**Goal:** establish the durable data contract independently of the new render algorithm.

**Owner:** SceneDB/API agent; isolated worktree; no render-pass implementation.

**Scope:** define reflected `VoxelComponent` and `VoxelTerrainComponent` properties/types; distinguish inspector properties from internal GPU-projection fields; establish stable identity/revision rules; define canonical chunk/edit storage and batch transaction semantics; provide external source registration and bulk publish/update APIs; validation, atomic commit, stale revision rejection, back-pressure/error reporting, persistence and undo hooks as supported by actual architecture.

**Must not:** put canonical state in renderer, add generic terrain branches to Helio, invent a second material system, or implement an unbounded synchronous upload/write loop.

**Deliverables:** API/type proposal checked against actual macros; implementation and focused tests; usage example showing script creates the component and publishes many chunks in batches; documented consistency and failure semantics.

**Done when:** a non-rendering test can create/configure both components, publish/read/update a batch, reject stale writes, and demonstrate that durable data is recoverable from SceneDB alone.
