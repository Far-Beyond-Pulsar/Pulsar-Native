...
Phases 0–2 are complete. Phase 3 is in progress: the committed Helio voxel-mesh chunk contract provides validated revisioned batches, but canonical chunk storage and the external read/publish/exfiltration API on SceneDB component state are still missing. Generic catalog and filesystem-payload prototypes are not the target persistence architecture and must not be treated as required engine storage. Continue Phase 3 with the SceneDB component data/batch API contract on `codex/unified-voxel-integration`; keep its write scope separate from render-pass algorithms. Do not merge the upstream branch. Keep the proprietary game outside the repository and defer workload integration details until performance qualification.
