...
- `VoxelComponent` and `VoxelTerrainComponent` carry runtime-only live payload maps and a data revision alongside authoring configuration. Payloads are not inspector properties or GPU mirrors, and normal component serialization does not claim to save them. The fields are private; `payload_store()` remains a low-level capability for the pass and should not be used by producers to bypass validation.
- `VoxelChunkBatch` validates key/domain/schema/revision/duplicate/size rules. `VoxelSourceWriter` applies a whole batch under one component-store lock, exposes immutable snapshots for caller-controlled exfiltration, and provides `replace_from_snapshot` for an explicit in-memory import/replay. Neither API serializes data or chooses durability policy.
- `BoundedVoxelInbox` supplies nonblocking `try_submit`, finite batch/operation/byte bounds, FIFO whole-batch draining under caller budgets, back-pressure outcomes, and close/drain/discard behavior. Its `Arc` payloads are transient pending work only. Drained batches are applied explicitly by the consumer via `publish_into`; this synchronous publication must be scheduled away from the render thread.
- Component `Clone` preserves the live data value while copying the mutable map/revision and sharing immutable `Arc` payload allocations. Tests cover cloned-map isolation and snapshots.
- The integration test now exercises the actual SceneDB row end to end: hydrate component -> obtain its store -> enqueue/drain/publish -> read snapshot -> import into a second component row -> reject stale work. The caller-owned export/import responsibility is documented in [`voxel-component-api-example.md`](voxel-component-api-example.md). No save settings, durable sidecar, automatic restart recovery, or built-in undo journal were added.
- Verification: `cargo test --locked -p helio-pass-voxel-mesh` passes 26 unit tests, 1 opaque generic-buffer contract test, and 1 portability test. `cargo test --locked -p engine_backend --test voxel_component_schema` passes all 5 tests, including the SceneDB-row round trip above.

Known correctness/performance boundaries carried into later phases: the low-level store capability can still bypass the writer if misused; writer identity is supplied by its caller rather than resolved from an entity; queue limits bound pending work, not cumulative canonical live memory; and drained batches are caller-owned until successfully published (there is no automatic retry/requeue). Canonical storage currently uses one blocking `RwLock<HashMap<...>>` per component. Batch application is synchronous; snapshots clone one `Arc` handle per entry while briefly holding a read lock and pin replaced payload memory until dropped. Large batches, snapshot creation, and imports belong on producer/script threads. These APIs do not establish the 10 ms frame target; qualification and any sharding/streaming redesign belong to later performance work.
